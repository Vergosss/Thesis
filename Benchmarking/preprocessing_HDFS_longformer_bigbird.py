...
##
###Tokenizing###
event_traces_train = event_traces_train.map(tokenize_logs,batched=True,num_proc=,load_from_cache_file=False)
event_traces_validation = event_traces_validation.map(tokenize_logs,batched=True,num_proc=,load_from_cache_file=False)
event_traces_test = event_traces_test.map(tokenize_logs,batched=True,num_proc=,load_from_cache_file=False)
####################
####Save to Disk for reuse############
event_traces_train.save_to_disk(f'/storage/data2/up1072604/data/tokenized_HDFS_train')
event_traces_validation.save_to_disk(f'/storage/data2/up1072604/data/tokenized_HDFS_validation')
event_traces_test.save_to_disk(f'/storage/data2/up1072604/data/tokenized_HDFS_test')
####
